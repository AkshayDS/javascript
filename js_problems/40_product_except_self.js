// Problem: Product of Array Except Self
// Given an integer array nums, return an array answer such that answer[i] is equal 
// to the product of all the elements of nums except nums[i].
// The product of any prefix or suffix of nums is guaranteed to fit in a 32-bit integer.
// You must write an algorithm that runs in O(n) time and without using the division operation.
// Example: [1,2,3,4] -> [24,12,8,6]

function productExceptSelf(nums) {
  const n = nums.length;
  const result = new Array(n).fill(1);
  
  // Step 1: Calculate left products
  // result[i] will contain the product of all elements to the left of i
  let leftProduct = 1;
  for (let i = 0; i < n; i++) {
    result[i] = leftProduct;
    leftProduct *= nums[i];
  }
  
  // Step 2: Calculate right products and multiply with left products
  // We compute the right product on the fly to save space
  let rightProduct = 1;
  for (let i = n - 1; i >= 0; i--) {
    result[i] *= rightProduct;
    rightProduct *= nums[i];
  }
  
  return result;
}

// O(N^2) naive approach (just for comparison, though requirement says O(N))
function productExceptSelfNaive(nums) {
    const result = [];
    for (let i = 0; i < nums.length; i++) {
        let product = 1;
        for (let j = 0; j < nums.length; j++) {
            if (i !== j) {
                product *= nums[j];
            }
        }
        result.push(product);
    }
    return result;
}

// Test cases
console.log(productExceptSelf([1, 2, 3, 4])); 
// [24, 12, 8, 6]

console.log(productExceptSelf([-1, 1, 0, -3, 3])); 
// [0, 0, 9, 0, 0]

console.log(productExceptSelf([0, 0])); 
// [0, 0]

console.log(productExceptSelf([1, 2])); 
// [2, 1]

console.log("\nNaive approach:");
console.log(productExceptSelfNaive([1, 2, 3, 4]));
// [24, 12, 8, 6]
